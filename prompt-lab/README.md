...
Prompt Lab 是 `change-audit` 的仓库级开发工具，用于验证 reviewer prompt 质量。它只做两件事：
...
```text
change_audit/review/core/reviewer-prompt.md
```

当前版本为 `product/v0.3`。`prompt-lab/run.py` 与产品 `prepare` 读取同一文件；Prompt Lab 不维护第二份实验模板，避免 prompt 版本和协议分叉。
...
python prompt-lab/eval.py \
  ...
  --output /tmp/change-audit-eval.json
```
