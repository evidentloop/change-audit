...
> Wave 0 至 Wave 3 的本地实现与门禁、Wave 4.1 clean 本地产物安装和 Wave 4.2 Codex 真实审计 E2E 已完成；当前停在 Wave 4.3 外部试跑 checkpoint。Wave 6 发布 checkpoint 通过前不得改名远端 repository、购买域名、配置 PyPI、push tag、发布或启用 Pages。分支实现的 commit/push 由用户单独授权，不等同于发布授权。
...
- [ ] 4.3 用本地 wheel 与 Skill 找至少一名外部试用者按 `docs/alpha-trial.md` 独立走通首次安装与一句话审计，记录环境、耗时、阻塞、误解与脱敏反馈；不收集源码，不要求 evidence worktree 或公开发布。
