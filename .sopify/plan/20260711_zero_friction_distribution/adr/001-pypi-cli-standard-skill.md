...
change-audit 是单一、聚焦的开源审计产品。当前 Python 内核已经成熟，但外部用户需要 clone、editable install 和手工 Skill 注册。自研 npm launcher、独立二进制或 Skill-first 平铺都会扩大维护面。

## 决策

- Python package/CLI 是唯一 runtime 与产品版本真相源，通过 PyPI 发布，uv tool 为推荐安装方式，pipx 为 fallback。
- Agent Skill 是同仓库 `skills/change-audit/` 下的静态薄编排层，通过通用 `skills` CLI 安装。
...
项目需要新增正式 console script、doctor、replay demo、标准 Skill、最小权限 publish workflow、Pages 入口和 PyPI Trusted Publishing。产品身份遵循 ADR-002，main/evidence 内容边界遵循 ADR-003。
