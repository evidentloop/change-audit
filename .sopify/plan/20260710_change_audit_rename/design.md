...
- 产品架构图文件名迁移为 `change-audit-architecture.*`；SVG 只替换可见 `<text>` 节点中的产品名，不改 `id`、`class`、`data-*` 等非可见属性，图中的 Typed Audit Graph 术语保留。
...
- 名称验证: `rg` 结果中，旧产品名只允许出现在历史方案、rename note、内部模型语境和保留产物名。
