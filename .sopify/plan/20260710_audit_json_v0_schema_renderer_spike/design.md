...
- `complete + pass_candidate + 0 open findings`：hero 显示“未发现未解决问题”；没有历史 finding 时不渲染空问题模块，有 fixed finding 时可以展示已解决记录。
- `not_reviewed`、`partial`、`failed`：hero 明示状态，不得显示为干净审查。
- 只有降级风险：渲染 Risks section，显示未定位标签，风险分显示“无法可靠评分”。
- 有 finding：展示 category、severity、可信 hunk、evidence、fix 和反馈控件；代码证据使用接近 diff2html 的双行号 hunk table，明确区分 context/add/delete、突出命中行并保留 `data-*` 回链，禁止用普通 `<pre>` 代替可信 diff hunk。
