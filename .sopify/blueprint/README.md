...
当前焦点是[产品身份与分发方案](../plan/20260711_identity_and_distribution/background.md)的 Wave 4；macOS arm64、Python 3.11 下的 uv wheel 与 pipx sdist 安装及断网运行已验证，Codex CLI `0.144.1` 和 `0.144.3` 的独立 reviewer JSONL 与正式报告对也已通过。Codex 支持按 capability gate 判断，不精确锁定 CLI 版本；Wave 4.3 当前只等待基于 `74c7d16` clean archive 的最小复跑，补齐候选产物 provenance 与精确耗时。
