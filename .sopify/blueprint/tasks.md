...
- [ ] 身份 checkpoint 通过后，将静态薄 Skill 迁到活动身份的唯一目录（EvidentLoop 目标为 `skills/evidentloop/`），声明 CLI/schema/prompt 兼容范围，并在不兼容时于 `prepare` 前 fail closed。
- [ ] 用 `skills@latest` 验证仓库嵌套 Skill 与辅助文件可完整安装；不自研 npm launcher、独立二进制、宿主 adapter、MCP 或模型 provider 层。
- [ ] 按身份 checkpoint 确认的唯一 CLI 重写 README 首次成功路径为“在线看 -> `uvx` demo -> uv/pipx 安装 -> 标准 skills CLI -> 一句话审计”，并用 GitHub Pages `audit-evidence/docs` 发布轻量入口。
- [ ] 验证 uv、pipx clean install 和至少一个真实宿主的 `Skill discovery -> isolated review -> finalize`；只声明实测通过的宿主与平台。
- [ ] 按身份 checkpoint 确认唯一 PyPI 名称与 Trusted Publisher，发布前创建同版本不可变 Git tag；GitHub Release 页面可选，未经授权不发布。
