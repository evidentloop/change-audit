...
- Python 包不调用模型 SDK、不读取 provider/API key 配置、不自动修改代码；宿主 LLM 是唯一模型执行面。
...
| `review-pack.json` | prepare | finalize | 内部 ReviewPack |
...
- Wave 5 删除仅为迁移等价性保留、但未进入正式链路的 provider-backed reviewer、配置、内部 CLI、formatter 与 provider budget/verify 路径；不建立新命名的兼容层。
- eval harness 移到 `prompt-lab/` 并保持离线 fixture 评估能力，不作为 wheel 运行时模块。
