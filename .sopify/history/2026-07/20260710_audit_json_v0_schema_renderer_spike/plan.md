...
- Status: Wave 0A/0B/1/2/3/4 与最终验证 7.1–7.5 已通过并由 PR #1 合并至 `main@f003337`；Wave 5 单产品收口的实现、clean wheel、同 range `product/v0.3` dogfood、localhost DOM、文档和两阶段复审均已通过，最终证据为 `verify_027.json`。
- Next: 方案已经归档；提交、push 并创建最小收口 PR。不创建 tag、Release，不处理旧仓库。
...
两者本来就是一条链路的上下游。继续分成两个 alpha 包会让用户安装两次、维护两套 Skill，并自行拼接审查结果与报告。合并后以 `change-audit` 作为唯一产品名，迁入的有效能力直接组成内部 `change_audit.review`；CrossReview 只保留为迁移来源和历史证据。
...
- **Wave 5 — Single-product consolidation**：删除未使用的 standalone provider/CLI/config，收缩 wheel 与 Prompt Lab，统一产品命名并用相同 Fireworks range 重新验证 prompt `product/v0.3`。
...
3. **宿主 LLM 是唯一模型执行面。** Python 包不调用模型 SDK、不解析 provider/API key 配置；迁移期保留的 standalone reviewer、内部 CLI 与配置层在 Wave 5 删除。
...
实施顺序以 [`tasks.md`](./tasks.md) 为唯一任务真相：Wave 0A 基线 → Wave 0B 等价迁移 → Wave 1 Schema + Renderer → Wave 2 Host Review Integration → Wave 3 Human Decision → Wave 4 AI Host Discovery → 最终验证与知识同步 → Wave 5 Single-product consolidation。每个 Wave 必须先完成实现、验证、文档回执和两阶段复审，退出门禁通过后才进入下一阶段或方案归档。
...
- D113：change-audit 只保留宿主编排模型执行面；删除未被 `prepare/finalize/render` 使用的 standalone provider、API key 配置、内部 CrossReview CLI、human formatter 与 provider budget gate，不创建 `CHANGE_AUDIT_*` 替代配置或兼容层。
- D114：ReviewPack、ReviewResult、pack、canonical prompt、normalizer、adjudicator 与 ingest 直接构成 `change_audit.review` 内核；结果构造并入 ingest。eval harness 属于仓库开发工具，迁至 `prompt-lab/`，不进入正式 wheel；Prompt Lab 直接复用 canonical prompt，不再维护第二份实验模板。
- D115：移除 `anthropic` optional extra；PyYAML 仅供 Prompt Lab/eval 使用并移入 dev extra。正式 wheel 只承担可信 diff、审查结果 ingest、Audit Graph、校验和 renderer。
- D116：canonical prompt 运行态名称统一为 change-audit 并升级为 `product/v0.3`；任何内容变化都必须重新走相同 Fireworks range 的 `prepare -> isolated host review -> finalize`，不得复用旧 raw analysis 或手改正式产物。历史 ADR、回执和归档来源说明可保留 CrossReview 名称。
- D117：Wave 5 新 dogfood 的 `audit.html` 直接采用 finalize 原生输出，不在产品链路外注入样张 SVG 或维护双哈希；现有 `docs/examples/dogfood-fireworks-tech-graph/` 按用户决定保持字节级不变，新 v0.3 证据先在 `/private/tmp` 验证，通过后独立写入 `docs/examples/dogfood-fireworks-tech-graph-v03/` 作为本地对比证据。
- D118：`prepare` 与 Prompt Lab 共用唯一 `render_host_reviewer_prompt()` 安全构造器，Skill 兼容探针同时校验 schema `0.2` 与 prompt `v0.3`；换行文件名在 prompt 文件列表中转义。删除 `review/pack.py` 中只服务旧内部 CLI、且与正式 Git adapter 重复的 Git/File I/O helper，不保留替代入口。
...
- [x] 7.5 最小 CI 与 MIT `LICENSE` 已通过 `verify_022.json`，PR #1 已合并至 `main@f003337`，main CI 成功。
- [x] Wave 5 Single-product consolidation：standalone 与 dead pack I/O 已删除，Prompt Lab、Skill 探针和安全构造器已统一；最终 316 项测试、Ruff、JavaScript、Python 3.12 clean wheel 隔离安装、同 range Fireworks v0.3 dogfood、localhost 1280/375 DOM、文档与两阶段复审全部通过。

## Next

本方案已经归档；后续只创建最小收口 PR。tag、Release、PyPI 发布和迁移来源仓库处理仍需单独授权。
