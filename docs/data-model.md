...
本文定义首个 `code_diff` audit profile 的 `audit.json 0.2-alpha`。它是该 profile 的唯一机器真相源，结构由 JSON Schema 2020-12 定义；Python 只补充跨对象引用、hunk anchor 和渲染回链校验。它不是所有 artifact 类型的预先通用 schema。
...
| `hunk` | 从 hunk index 复制的完整 render-ready snippet |
...
| 完整且无未解决 finding | complete | pass_candidate |
...
- severity 权重集中配置并封顶 100，在 Wave 2 dogfood 后冻结。
...
```json
{"target_type":"finding","target_id":"finding-001","action":"false_positive","reason":"缓存已有统一刷新保障","fingerprint":"sha256:...","created_at":"2026-07-10T10:30:00+08:00"}
```
...
- finding 的代码证据必须来自 `audit.json` 中由可信 hunk index 复制的完整 hunk，并渲染为接近 diff2html 的双行号表格，明确区分 context/add/delete、突出命中行；普通 `<pre>` 不能替代可信 diff hunk。
- 对业务文本统一转义。
- 所有 finding、edge、claim 和 feedback target 保留 `data-*` 回链。
...
run-003: 完整审查且无未解决 finding，pass_candidate
```
