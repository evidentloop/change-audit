...
| Qoder | 已验证固定 wheel 安装 | 已试跑，未保留独立 discovery 证据 | 阻塞：独立 reviewer 隔离未验证 |
| 其他宿主 | 未验证 | 未验证 | 未验证 |

Codex 的验证使用独立 `codex exec`、不同 thread ID、无工具事件的 reviewer JSONL、空工作目录和临时目录清理断言。这些是 Codex 证据映射，不是其他宿主必须复制的字段或目录。

Qoder 外部试跑通过固定 provenance、安装、doctor、prepare/finalize 机械链路和产物 identity，但无法提供可验证的独立 reviewer。执行者在该门禁缺失后仍调用 `finalize`，因此生成的 `partial / inconclusive` 产物只作为诊断，不构成真实审计 E2E。
