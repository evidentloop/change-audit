...
|---|---|---|---|
| Codex | 已验证 | 已验证 | 已验证（macOS、Codex CLI `0.144.1`） |
| 其他宿主 | 未验证 | 未验证 | 未验证 |

Codex 验证环境为 macOS arm64、Python `3.11.15`、Codex CLI `0.144.1`、uv `0.11.28` 和 skills CLI `1.5.16`。验证使用全新 HOME、本地 wheel 和复制安装到 `$HOME/.agents/skills/evidentloop` 的 Skill。一句自然语言触发后，orchestrator 与 reviewer 使用不同 thread ID；reviewer JSONL 无工具事件，最终报告为 `complete / concerns`，并把已知语义缺陷精确锚定到 `billing.py:3`。该结果只覆盖上述实测组合，不扩大其他平台或宿主支持范围。
...
Codex CLI `0.144.1` 的实测路径使用单独的 `codex exec` 进程、全新 HOME、只含传输认证的临时可写 `CODEX_HOME`、空工作目录和只读 sandbox，并关闭 shell、浏览器、MCP、插件及协作工具。宿主必须比较 orchestrator 与 reviewer 的 `thread.started` ID，要求 reviewer JSONL 恰有一个最终 `agent_message` 和 `turn.completed`，且没有任何工具事件；临时 HOME、`CODEX_HOME` 和工作目录必须在 `finalize` 前删除。任一条件不满足就在 `finalize` 前停止。完整命令以 [`skills/evidentloop/SKILL.md`](../skills/evidentloop/SKILL.md) 为准。
