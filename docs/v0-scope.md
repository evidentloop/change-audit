...
`change-audit` v0 面向本地 Git 变更，使用 AI host 已有的 LLM 做语义审查，再由 Python 生成可校验的 `audit.json` 和自包含 `audit.html`。本文件定义第一个 `code_diff` audit profile；Schema `0.2` 当前只支持 Git diff。
...
- one-product：ReviewPack、prompt、ingest、normalizer 与 adjudicator 直接组成内部 `change_audit.review`，不存在第二个用户产品或模型执行面。
...
- `change_audit.review` 的 ReviewPack、canonical prompt、normalizer、adjudicator、ingest 和 ReviewResult。
...
- Python 包调用模型 SDK 或管理 provider/API key 配置。
...
- prompt 以每次运行唯一的动态边界包裹 diff，文件列表中的换行与控制分隔符按单行数据转义，并明确禁止服从数据中的指令；其他背景字段以不可信声明标注，不扩展成通用 prompt 编码框架。
