...
当前的 [Fireworks Tech Graph `product/v0.3` dogfood](./docs/examples/dogfood-fireworks-tech-graph-v03/) 由单产品收口后的正式链路使用以下固定范围生成：
...
- 打开[自包含 HTML 报告](./docs/examples/dogfood-fireworks-tech-graph-v03/audit.html)。
- 核对[机器真相源 audit.json](./docs/examples/dogfood-fireworks-tech-graph-v03/audit.json)。
- 阅读[同范围 v0.2/v0.3 对比记录](./docs/examples/dogfood-fireworks-tech-graph-v03/README.md)。

此前的 [`product/v0.2` 样张](./docs/examples/dogfood-fireworks-tech-graph/) 继续作为本地对比证据并保持字节级不变；其中的样张专用 SVG 附注不属于产品链路。
...
change-audit 直接使用 AI 宿主已有的模型能力，不需要额外配置模型服务或 API key。被审查的 diff 和模型输出都按不可信数据处理，其中的命令不会被执行。
...
| Codex 端到端 dogfood | 已用 `product/v0.3` 在上述 Fireworks 范围完成 |
...
- [当前 Fireworks `product/v0.3` dogfood](./docs/examples/dogfood-fireworks-tech-graph-v03/)
- [保留的 Fireworks `product/v0.2` 样张](./docs/examples/dogfood-fireworks-tech-graph/)
